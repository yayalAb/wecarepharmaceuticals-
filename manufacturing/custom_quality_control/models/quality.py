...
class QualityPoint(models.Model):
    ...
    def action_see_quality_checks(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("custom_quality_control.quality_check_action_main")
    ...
    def action_see_spc_control(self):
        self.ensure_one()
        action = self.env["ir.actions.actions"]._for_xml_id("custom_quality_control.quality_check_action_spc")
...
class QualityCheck(models.Model):
    ...
    def do_alert(self):
        ...
        return {
            ...
            'views': [(self.env.ref('custom_quality_control.quality_alert_view_form').id, 'form')],
            'res_id': alert.id,
    ...
    def action_see_alerts(self):
        ...
        if len(self.alert_ids) == 1:
            ...
            action = self.env["ir.actions.actions"]._for_xml_id("custom_quality_control.quality_alert_action_check")
    ...
    def action_open_quality_check_wizard(self, current_check_id=None):
        ...
        action = self.env["ir.actions.actions"]._for_xml_id("custom_quality_control.action_quality_check_wizard")
