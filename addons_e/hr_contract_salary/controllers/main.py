...
from odoo.addons.sign_custom.controllers.main import Sign
...
class SignContract(Sign):
    ...
    def sign(self, sign_request_id, token, sms_token=False, signature=None, **kwargs):
        result = super().sign(sign_request_id, token,
        ...
        if result.get('success'):
            ...
            if offer.state in ['expired', 'refused']:
                raise UserError(
                    _('This offer is outdated, please request an updated link...'))
            ...
            if contract and request_template_id in contract_documents:
                self._update_contract_on_signature(
                    request_item, contract, offer)
        ...
        for benefit in benefits:
            ...
            if (benefit.activity_creation_type == "onchange" and contract[field] != contract.origin_contract_id[field]) or \
                ...
                contract.activity_schedule(
                    ...
                    note="%s: %s" % (
                        benefit.name or benefit.res_field_id.name, value),
    ...
    def _send_benefit_sign_request(self, contract):
        ...
        SignRequestSudo = request.env['sign.request'].with_user(
            contract.hr_responsible_id).sudo()
        ...
        for benefit in benefits:
            ...
            if (benefit.activity_creation_type == "onchange" and contract[field] != contract.origin_contract_id[field]) or \
                ...
                if request.env.ref('sign_custom.sign_item_role_employee') in template_roles:
                    request_items.append(Command.create({'role_id': request.env.ref('sign_custom.sign_item_role_employee').id,
                ...
                sign_request_sudo.message_subscribe(
                    partner_ids=benefit.sign_copy_partner_id.ids)
...
class HrContractSalary(http.Controller):
    ...
    def _check_access_rights(self, contract_id):
        ...
        contract = request.env['hr.contract'].with_context(
            allowed_company_ids=request.env.user.company_ids.ids).browse(contract_id)
    ...
    def salary_package(self, offer_id=None, **kw):
        ...
        for bundle_name in ["web.assets_frontend", "web.assets_frontend_lazy"]:
            request.env["ir.qweb"]._get_asset_nodes(
    ...
    def _get_personal_infos(self, contract):
        ...
        mapped_personal_infos = [
            # Main Panel
            defaultdict(
                lambda: request.env['hr.contract.salary.personal.info']),
        ...
        for personal_info in personal_infos:
            ...
            if personal_info.display_type == 'document':
                ...
                if filename_field in contract._fields:
                    initial_values[personal_info.field +
                                   '_filename'] = contract[filename_field]
                elif filename_field in target:
                    initial_values[personal_info.field +
                                   '_filename'] = target[filename_field]
                else:
                    initial_values[personal_info.field +
                                   '_filename'] = personal_info.field
            else:
            ...
            if personal_info.display_type == 'dropdown':
                ...
                if personal_info.dropdown_selection == 'specific':
                    ...
                    values = [(country.id, country.name) for country in self._get_personal_infos_countries(
                        contract, personal_info)]
                elif personal_info.dropdown_selection == 'state':
                    values = [(state.id, state.name, state.country_id.id)
                              for state in self._get_personal_infos_states(contract, personal_info)]
                elif personal_info.dropdown_selection == 'lang':
                    values = [(lang.code, lang.name) for lang in self._get_personal_infos_langs(
                        contract, personal_info)]
    ...
    def _get_benefits_values(self, contract, offer):
        ...
        mapped_benefits = defaultdict(
            lambda: request.env['hr.contract.salary.benefit'])
        for benefit in benefits:
        ...
        dropdown_benefits = benefits.filtered(
            lambda a: a.display_type == 'dropdown')
        for dropdown_benefit in dropdown_benefits:
            dropdown_options[dropdown_benefit.field] = \
                [(value.value, value.value) for value in dropdown_benefit.value_ids.filtered(
                    lambda v: v.display_type == 'line')]
        dropdown_group_benefits = benefits.filtered(
            lambda a: a.display_type == 'dropdown-group')
        ...
        benefit_types = sorted(benefits.mapped(
            'benefit_type_id'), key=lambda x: x.sequence)
        ...
        for dependent_benefit in benefits:
            if not dependent_benefit.field:
                continue
            mapped_mandatory_benefits_names[dependent_benefit] = (
                dependent_benefit.fold_label or dependent_benefit.name) + ';'
            ...
            for mandatory_benefit in dependent_benefit.benefit_ids:
                ...
                mapped_mandatory_benefits_names[dependent_benefit] += (
                    mandatory_benefit.fold_label or mandatory_benefit.name) + ';'
    ...
    def _get_salary_package_values(self, contract, offer):
        mapped_personal_infos, dropdown_options_1, initial_values_1 = self._get_personal_infos(
            contract)
        mapped_benefits, mapped_dependent_benefits, mandatory_benefits, mandatory_benefits_names, benefit_types, dropdown_options_2, dropdown_group_options, initial_values_2 = self._get_benefits_values(
            contract, offer)
        all_initial_values = {**initial_values_1, **initial_values_2}
        all_initial_values = {key: round(value, 2) if isinstance(
            value, float) else value for key, value in all_initial_values.items()}
    ...
    def _get_new_contract_values(self, contract, employee, benefits, offer):
        ...
        for benefit in contract_benefits:
            ...
            if hasattr(contract, '_get_benefit_values_%s' % (benefit.field)):
                contract_vals.update(getattr(contract, '_get_benefit_values_%s' % (
                    benefit.field))(contract, benefits))
                continue
            if benefit.folded:
                contract_vals[benefit.fold_field or benefit.field] = benefits['fold_%s' % (
                    benefit.field)]
            if benefit.display_type == 'dropdown':
                contract_vals[benefit.field] = benefits[benefit.field]
            if benefit.display_type in ['manual', 'text']:
                contract_vals[benefit.manual_field or benefit.field] = benefits['%s_%s' % (
                    benefit.field, 'manual' if benefit.display_type == 'manual' else 'text')]
            else:
                contract_vals[benefit.field] = benefits[benefit.field]
    ...
    def _update_personal_info(self, employee, contract, personal_infos_values, no_name_write=False):
        ...
        job = request.env['hr.job'].sudo().browse(
            employee_infos['employee_job_id'])
        if not employee_infos['job_title']:
            employee_infos['job_title'] = job.name

        if employee.department_id.parent_path:
            employee_department = employee.department_id.id if str(
                employee_infos['department_id']) in employee.department_id.parent_path.split('/') else employee_infos['department_id']
        else:
        ...
        for personal_info in personal_infos:
            ...
            if field_name in employee_infos and personal_info.applies_on == 'employee':
                employee_vals[field_name] = resolve_value(
                    field_name, employee_infos)
            elif field_name in bank_account_infos and personal_info.applies_on == 'bank_account':
                bank_account_vals[field_name] = resolve_value(
                    field_name, bank_account_infos)
        ...
        if employee.work_contact_id:
            ...
            partner = request.env['res.partner'].sudo().with_context(
                lang=None, tracking_disable=True).create(work_contact_vals)
    ...
    def create_new_contract(self, contract, offer_id, benefits, no_write=False, **kw):
        ...
        offer = request.env['hr.contract.salary.offer'].sudo().browse(
            offer_id).exists()
        applicant = offer.applicant_id
        employee = kw.get(
            'employee') or contract.employee_id or applicant.employee_id
                ...
                for field in personal_infos[section]:
                    if field in employee_fields:
                        ...
                        if isinstance(current_value, type(new_value)) and current_value == new_value:
                            ...
                            if new_value:
                                new_record = request.env[employee_fields[field].comodel_name].sudo().browse(
                                    int(new_value))
                            ...
                            current_value = current_value.strftime(
                                '%Y-%m-%d') if current_value else ''
                        ...
                        if current_value != new_value:
                            ...
                            contract_diff.append(
                                (employee_field_name, current_value, new_value))

        self._update_personal_info(
            employee, contract, personal_infos, no_name_write=bool(kw.get('employee')))
        ...
        if no_write:
            ...
            for field in contract_fields:
                if field in contract_values and contract[field] != new_contract[field]\
                    ...
                    contract_diff.append(
                        (contract_field_name, current_value, new_value))

        if 'original_link' in kw:
            start_date = parse_qs(urlparse(kw['original_link']).query).get(
                'contract_start_date', False)
            if start_date:
                new_contract.date_start = datetime.strptime(
                    start_date[0], '%Y-%m-%d').date()

        new_contract.wage_with_holidays = contract_values['wage']
        new_contract.final_yearly_costs = float(
            contract_values['final_yearly_costs'] or 0.0)
    ...
    def update_salary(self, contract_id=None, offer_id=None, benefits=None, **kw):
        ...
        new_contract = self.create_new_contract(
            contract, offer_id, benefits, no_write=True)[0]
        final_yearly_costs = float(
            benefits['contract']['final_yearly_costs'] or 0.0)
        new_gross = new_contract._get_gross_from_employer_costs(
            final_yearly_costs)
        new_contract.write({
            'wage': new_gross,
    ...
    def _get_compute_results(self, new_contract):
        ...
        result['resume_categories'] = [c.name for c in sorted(
            resume_lines.mapped('category_id'), key=lambda x: x.sequence)]
        result['resume_lines_mapped'] = defaultdict(lambda: {})

        monthly_total = 0
        monthly_total_lines = resume_lines.filtered(
            lambda l: l.value_type == 'monthly_total')

        uoms = {'days': _('Days'), 'percent': '%', 'currency': new_contract.company_id.currency_id.symbol,
                'position': new_contract.company_id.currency_id.position}
        ...
        for resume_line in resume_lines - monthly_total_lines:
            ...
            if resume_line.impacts_monthly_total:
                monthly_total += value / \
                    12.0 if resume_line.category_id.periodicity == 'yearly' else value
            try:
                value = round(float(value), 2)
            except:
                pass
            result['resume_lines_mapped'][resume_line.category_id.name][resume_line.code] = (
                resume_line.name, value, uom, resume_explanation, new_contract.company_id.currency_id.position, resume_line.uom)
        for resume_line in monthly_total_lines:
            result['resume_lines_mapped'][resume_line.category_id.name][resume_line.code] = (resume_line.name, round(
                float(monthly_total), 2), uoms['currency'], uoms['position'], resume_explanation, resume_line.uom)
        return result
    ...
    def onchange_benefit(self, benefit_field, new_value, contract_id, benefits):
        ...
        if hasattr(contract, '_get_description_%s' % benefit_field):
            description = getattr(
                contract, '_get_description_%s' % benefit_field)(new_value)
        else:
    ...
    def onchange_personal_info(self, field, value):
        ...
        info = request.env['hr.contract.salary.personal.info'].sudo().search([
            ('field', '=', field)])
    ...
    def _get_email_info(self, contract, **kw):
        ...
        contract_benefits = request.env['hr.contract.salary.benefit'].sudo().search(
            [('structure_type_id', '=', contract.structure_type_id.id)])
        contract_info = {benefit_type.name: [] for benefit_type in sorted(
            contract_benefits.mapped('benefit_type_id'), key=lambda x: x.sequence)}
        for benefit in contract_benefits:
            if benefit.folded and benefit.fold_field:
                value = _('Yes') if contract[benefit.fold_field] else _('No')
                contract_info[benefit.benefit_type_id.name].append(
                    (field_names['hr.contract'][benefit.fold_field], value))
            ...
            contract_info[benefit.benefit_type_id.name].append(
                (field_names['hr.contract'][field_name], field_value))
        ...
        infos = request.env['hr.contract.salary.personal.info'].sudo().search([('display_type', '!=', 'document'), '|', (
            'structure_type_id', '=', False), ('structure_type_id', '=', contract.structure_type_id.id)])
        personal_infos = {personal_info_type.name: [] for personal_info_type in sorted(
            infos.mapped('info_type_id'), key=lambda x: x.sequence)}
        for info in infos:
            ...
            if isinstance(field_value, models.BaseModel):
                ...
                field_value = round(field_value, 2)
            personal_infos[info.info_type_id.name].append(
    ...
    def submit(self, contract_id=None, offer_id=None, benefits=None, **kw):
        ...
        if not offer.applicant_id and not offer.employee_contract_id:
            raise UserError(
                _('This link is invalid. Please contact the HR Responsible to get a new one...'))
        ...
        new_contract = self.create_new_contract(
            contract, offer_id, benefits, no_write=True, **kw)
        ...
        if applicant and offer.access_token:
            hash_token_access = hashlib.sha1(
                kw.get('token').encode("utf-8")).hexdigest()
            existing_contract = request.env['hr.contract'].sudo().search([
                ('applicant_id', '=', applicant.id), ('hash_token', '=', hash_token_access), ('active', '=', False)])
            existing_contract.sign_request_ids.write(
                {'state': 'canceled', 'active': False})
            ...
            hash_token_access = hashlib.sha1(
                kw.get('original_link').encode("utf-8")).hexdigest()
            existing_contract = request.env['hr.contract'].sudo().search([
        ...
        SignRequestSudo = request.env['sign.request'].with_user(
            new_contract.hr_responsible_id).sudo()
        ...
        sign_values_by_role = defaultdict(
            lambda: defaultdict(lambda: request.env['sign.item']))
    ...
    def refuse(self, offer_id, feedback=None, token=None):
        ...
        contract = self._check_access_rights(
            offer.employee_contract_id.id) if offer.employee_contract_id else False
