...
class SignRequest(models.Model):
    ...
    def _refuse(self, refuser, refusal_reason):
        ...
        for user in self.request_item_ids.partner_id.user_ids.filtered(lambda u: u.has_group('sign_custom.group_sign_user')):
    ...
    def cancel(self):
        ...
        for user in self.request_item_ids.sudo().partner_id.user_ids.filtered(lambda u: u.has_group('sign_custom.group_sign_user')):
            self.activity_unlink(['mail.mail_activity_data_todo'], user_id=user.id)
...
class SignRequestItem(models.Model):
    ...
    def write(self, vals):
        ...
        if vals.get('partner_id'):
            ...
            for request_item in request_items_reassigned:
                ...
                if old_sign_user and old_sign_user.has_group('sign_custom.group_sign_user') and \
            ...
            new_sign_user = self.env['res.users'].search([
                ...
                ('groups_id', 'in', [self.env.ref('sign_custom.group_sign_user').id])
            ], limit=1)
            if new_sign_user:
    ...
    def _refuse(self, refusal_reason):
        ...
        if refuse_user and refuse_user.has_group('sign_custom.group_sign_user'):
    ...
    def _post_fill_request_item(self):
        ...
        if not self.sign_request_id.request_item_ids.filtered(lambda sri: sri.partner_id == self.partner_id and sri.state == 'sent'):
            ...
            if sign_user and sign_user.has_group('sign_custom.group_sign_user'):
    ...
    def send_signature_accesses(self):
        ...
        user_ids = set(users.sudo().search([('groups_id', 'in', self.env.ref('sign_custom.group_sign_user').id), ('id', 'in', users.ids)]).ids)
