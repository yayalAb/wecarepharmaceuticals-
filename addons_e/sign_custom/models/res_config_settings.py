...
class ResConfigSettings(models.TransientModel):
    ...
    sign_terms = fields.Html(related='company_id.sign_terms',
                             string="Sign Terms & Conditions", readonly=False)
    sign_terms_html = fields.Html(related='company_id.sign_terms_html', string="Sign Terms & Conditions as a Web page",
                                  readonly=False)
    qwen_api_key = fields.Char()

    sign_terms_type = fields.Selection(
        related='company_id.sign_terms_type', readonly=False)
    sign_preview_ready = fields.Boolean(
        string="Display sign preview button", compute='_compute_sign_terms_preview')

    use_sign_terms = fields.Boolean(
        string='Sign Default Terms & Conditions',
        config_parameter='sign_custom.use_sign_terms',
        default=False)

    group_manage_template_access = fields.Boolean(
        string="Manage template access", implied_group='sign_custom.manage_template_access')
