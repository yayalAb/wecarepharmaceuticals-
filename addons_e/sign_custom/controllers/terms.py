...
class TermsController(http.Controller):
    ...
    def sitemap_sign_terms(env, rule, qs):
        use_sign_terms = env['ir.config_parameter'].sudo(
        ).get_param('sign_custom.use_sign_terms')
    ...
    def terms_conditions(self, **kwargs):
        use_sign_terms = request.env['ir.config_parameter'].sudo(
        ).get_param('sign_custom.use_sign_terms')
        ...
        return request.render("sign_custom.sign_terms_conditions_page", values)
