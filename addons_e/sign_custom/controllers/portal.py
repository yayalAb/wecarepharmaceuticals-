...
class CustomerPortal(portal.CustomerPortal):
    ...
    def _prepare_home_portal_values(self, counters):
        values = super(
            CustomerPortal, self)._prepare_home_portal_values(counters)
        ...
        if 'sign_count' in counters:
            ...
            values['sign_count'] = request.env['sign.request.item'].sudo().search_count([
                ('partner_id', '=', partner_id.id), '|', ('state',
                                                          '=', 'completed'), ('is_mail_sent', '=', True)
            ])
        return values
    ...
    def portal_my_signatures(self, page=1, date_begin=None, date_end=None, sortby=None, search=None, search_in='all',
        ...
        if date_begin and date_end:
            domain = AND(
                [domain, [('signing_date', '>', date_begin), ('signing_date', '<=', date_end)]])
    ...
    def portal_my_signature(self, item_id, **kwargs):
        partner_id = request.env.user.partner_id
        sign_item_sudo = request.env['sign.request.item'].sudo().browse(
            item_id)
        ...
        return request.render('sign_custom.sign_portal_my_request', values)
