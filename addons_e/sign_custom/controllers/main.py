...
class Sign(http.Controller):
    ...
    def get_document_qweb_context(self, sign_request_id, token, **post):
        sign_request = http.request.env['sign.request'].sudo().browse(
            sign_request_id).exists()
        if not sign_request:
            return request.render('sign_custom.deleted_sign_request')
        current_request_item = sign_request.request_item_ids.filtered(
            lambda r: consteq(r.access_token, token))
        ...
        if current_request_item and current_request_item.partner_id.lang:
            http.request.env.context = dict(
                http.request.env.context, lang=current_request_item.partner_id.lang)

        sign_item_types = http.request.env['sign.item.type'].sudo().search_read([
        ])
        if not sign_item_types:
            raise UserError(
                _("Unable to sign the document due to missing required data. Please contact an administrator."))

        # Currently only Signature, Initials, Text are allowed to be added while signing
        item_type_signature = request.env.ref(
            'sign_custom.sign_item_type_signature', raise_if_not_found=False)
        item_type_initial = request.env.ref(
            'sign_custom.sign_item_type_initial', raise_if_not_found=False)
        item_type_text = request.env.ref(
            'sign_custom.sign_item_type_text', raise_if_not_found=False)
        ...
        if current_request_item:
            for item_type in sign_item_types:
                if item_type['auto_field']:
                    try:
                        auto_field = current_request_item.partner_id.mapped(
                            item_type['auto_field'])
                        item_type['auto_value'] = auto_field[0] if auto_field and not isinstance(
                            auto_field, models.BaseModel) else ''
                    except Exception:
                        item_type['auto_value'] = ''
                if item_type['item_type'] in ['signature', 'initial']:
                    signature_field_name = 'sign_signature' if item_type[
                        'item_type'] == 'signature' else 'sign_initials'
                    user_signature = current_request_item._get_user_signature(
                        signature_field_name)
                    user_signature_frame = current_request_item._get_user_signature_frame(
                        signature_field_name+'_frame')
                    item_type['auto_value'] = 'data:image/png;base64,%s' % user_signature.decode(
                    ) if user_signature else False
                    item_type['frame_value'] = 'data:image/png;base64,%s' % user_signature_frame.decode(
                    ) if user_signature_frame else False
        ...
        sr_values = http.request.env['sign.request.item.value'].sudo().search([('sign_request_id', '=', sign_request.id), '|', (
            'sign_request_item_id', '=', current_request_item.id), ('sign_request_item_id.state', '=', 'completed')])
    ...
    def share_link(self, share_link, **post):
        ...
        """
        sign_request_item = request.env['sign.request.item'].sudo().search(
            [('access_token', '=', share_link)], limit=1)
    ...
    def sign_document_from_mail(self, request_id, token, **post):
        ...
        if not sign_request or sign_request.validity and sign_request.validity < fields.Date.today():
            return http.request.render('sign_custom.deleted_sign_request', status=404)
        current_request_item = sign_request.request_item_ids.filtered(
            lambda r: consteq(r.access_token, token))
        ...
        removed_timestamp_arg = sign_request.state == 'sent' and (
            not post.get('timestamp') or not post.get('exp'))
        if sign_request.state != 'shared' and not current_request_item._validate_expiry(post.get('timestamp'), post.get('exp')):
            if removed_timestamp_arg:
                return http.request.render('sign_custom.deleted_sign_request', status=404)
            return request.render('sign_custom.sign_request_expired', {'resend_expired_link': '/sign/resend_expired_link/%s/%s' % (request_id, token)}, status=403)
    ...
    def sign_document_public(self, sign_request_id, token, **post):
        document_context = self.get_document_qweb_context(
            sign_request_id, token, **post)
        if not isinstance(document_context, dict):
            return document_context

        return http.request.render('sign_custom.doc_sign', document_context)
    ...
    def download_document(self, request_id, token, download_type, **post):
        ...
        if download_type == "log":
            ...
            pdf_content, __ = report_action._render_qweb_pdf(
                'sign_custom.action_sign_request_print_logs',
                sign_request.id,
        ...
        extension = '.' + sign_request.template_id.attachment_id.mimetype.replace(
            'application/', '').replace(';base64', '')
    ...
    def download_multiple_documents(self, ids, **post):
        ...
        """
        if not request.env.user.has_group('sign_custom.group_sign_user'):
            return request.render(
                'http_routing.http_error',
                {'status_code': _('Oops'),
                 'status_message': _('You do not have access to these documents, please contact a Sign Administrator.')})

        sign_requests = http.request.env['sign.request'].browse(
            int(i) for i in ids.split(',')).exists()
            ...
            with zipfile.ZipFile(buffer, 'w', compression=zipfile.ZIP_DEFLATED) as zipfile_obj:
                for sign_request in sign_requests:
                    ...
                    zipfile_obj.writestr(
                        f'{sign_request.id}/{sign_request.reference}', base64.b64decode(sign_request.completed_document))
    ...
    def check_password_page(self, sign_request_id, token, **post):
        ...
        if 'password' not in http.request.params:
            return http.request.render('sign_custom.encrypted_ask_password')

        password = http.request.params['password']
        template_id = request_item.sign_request_id.template_id

        old_pdf = PdfFileReader(io.BytesIO(base64.b64decode(
            template_id.attachment_id.datas)), strict=False, overwriteWarnings=False)
        if old_pdf.isEncrypted and not old_pdf.decrypt(password):
            values['error'] = _("Wrong password")
            return http.request.render('sign_custom.encrypted_ask_password', values)
    ...
    def resend_expired_link(self, request_id, token):
        ...
        if not sign_request or sign_request.state in ('signed', 'canceled', 'refused'):
            return http.request.render('sign_custom.deleted_sign_request')
        current_request_item = sign_request.request_item_ids.filtered(
            lambda r: consteq(r.access_token, token))
        ...
        return request.render('sign_custom.sign_request_expired', {
            'state': 'sent',
    ...
    def get_document(self, request_id, token):
        ...
        if not isinstance(context, dict):
            ...
            return context
        return {
            'html': request.env['ir.qweb']._render('sign_custom._doc_sign', context),
            'context': {
    ...
    def update_signature(self, sign_request_id, role, signature_type=None, datas=None, frame_datas=None):
        sign_request_item_sudo = http.request.env['sign.request.item'].sudo().search(
            [('sign_request_id', '=', sign_request_id), ('role_id', '=', role)], limit=1)
        ...
        user[signature_type +
             '_frame'] = frame_datas[frame_datas.find(',') + 1:] if frame_datas else False
        return True
    ...
    def new_partners(self, partners=[]):
        ...
        for p in partners:
            ...
            pIDs.append(existing.id if existing else ResPartner.create(
                {'name': p[0], 'email': p[1]}).id)
        return pIDs
    ...
    def make_public_user(self, request_id, token, name=None, mail=None):
        sign_request = http.request.env['sign.request'].sudo().search(
            [('id', '=', request_id), ('access_token', '=', token)])
    ...
    def send_sms(self, request_id, token, phone_number):
        request_item = http.request.env['sign.request.item'].sudo().search(
            [('sign_request_id', '=', request_id), ('access_token', '=', token), ('state', '=', 'sent')], limit=1)
        if not request_item:
            return False
        if request_item.role_id.auth_method == 'sms':
            ...
            try:
                ...
                request_item.sign_request_id.activity_schedule(
                    ...
                    note=_("%s couldn't sign the document due to an insufficient credit error.",
                           request_item.partner_id.display_name),
    ...
    def _validate_auth_method(self, request_item_sudo, sms_token=None):
        if request_item_sudo.role_id.auth_method == 'sms':
            has_sms_credits = request.env['iap.account'].sudo().get_credits(
                'sms') > 0  # credits > 0 because the credit was already spent
            ...
            request_item_sudo.sign_request_id._message_log(
                body=_('%(partner)s validated the signature by SMS with the phone number %(phone_number)s.',
                       partner=request_item_sudo.partner_id.display_name, phone_number=request_item_sudo.sms_number)
            )
            return {'success': True}
    ...
    def sign(self, sign_request_id, token, sms_token=False, signature=None, **kwargs):
        ...
        if request_item_sudo.role_id.auth_method:
            result = self._validate_auth_method(
                request_item_sudo, sms_token=sms_token)
            if not result.get('success'):
                return result

        sign_user = request.env['res.users'].sudo().search(
            [('partner_id', '=', request_item_sudo.partner_id.id)], limit=1)
        if sign_user:
    ...
    def refuse(self, sign_request_id, token, refusal_reason=""):
        ...
        refuse_user = request.env['res.users'].sudo().search(
            [('partner_id', '=', request_item.partner_id.id)], limit=1)
        if refuse_user:
    ...
    def check_encrypted(self, sign_request_id):
        request_item = http.request.env['sign.request.item'].sudo().search(
            [('sign_request_id', '=', sign_request_id)], limit=1)
        if not request_item:
            return False
    ...
    def save_location(self, request_id, token, latitude=0, longitude=0):
        sign_request_item = http.request.env['sign.request.item'].sudo().search(
            [('sign_request_id', '=', request_id), ('access_token', '=', token)], limit=1)
    ...
    def render_assets_pdf_iframe(self, **kw):
        ...
        return request.env['ir.ui.view'].sudo()._render_template('sign_custom.compiled_assets_pdf_iframe', context)
    ...
    def get_iap_credit_warnings(self):
        warnings = []
        roles_with_sms = request.env['sign.item.role'].sudo().search(
            [('auth_method', '=', 'sms')])
        if roles_with_sms:
    ...
    def get_sign_request_items(self, request_id, token):
        ...
        items = request.env['sign.request.item'].sudo().search_read(
            ...
            fields=['access_token', 'sign_request_id',
                    'create_uid', 'create_date'],
            order='create_date DESC',
            limit=20,
        )
        items.sort(key=lambda item: (
            0 if item['create_uid'] and uid == item['create_uid'][0] else 1))
        items = items[:3]
        return [{
            'id': item['id'],
    ...
    def confirm_cancel_sign_request_item(self, item_id, access_token=None):
        sign_request_item = request.env['sign_custom.request.item'].sudo().browse(
            item_id)
        if sign_request_item and consteq(sign_request_item.access_token, access_token):
            sign_request_item.sign_request_id.with_context(
                default_sign_request_item_id=sign_request_item.id).cancel()
            message_post = _("The signature has been canceled by %(partner)s(%(role)s)",
                             partner=sign_request_item.partner_id.name, role=sign_request_item.role_id.name)
            sign_request_item.sign_request_id.message_post(body=message_post)
            return http.request.render('sign_custom.canceled_sign_request_item')
        else:
            return http.request.not_found()
    ...
    def cancel_sign_request_item_from_mail(self, item_id, token):
        sign_request_item = request.env['sign.request.item'].sudo().browse(
            item_id)
        if sign_request_item and consteq(sign_request_item.access_token, token):
            if request.env.ref('sign_custom.cancel_sign_request_item_with_confirmation', raise_if_not_found=False):
                return http.request.render('sign_custom.cancel_sign_request_item_with_confirmation', {
                    'record': sign_request_item,
                })
            # Mail scanners can send HEAD requests to link contained in emails, it shouldn't cancel the request.
