...
class SuppliesRfpProductLine(models.Model):
    ...
    ordered_qty = fields.Float(
        string='Ordered Quantity', digits='Product Unit of Measure')
    remaining_qty = fields.Float(
        string='Remaining Quantity', compute='_compute_remaining_qty', store=True, digits='Product Unit of Measure')
    unit_price = fields.Monetary(string='Price')
    product_uom = fields.Many2one('uom.uom', string='UOM')
    subtotal_price = fields.Monetary(
        string='Subtotal', compute='_compute_subtotal_price', store=True)
    ...
    def _onchange_product_id(self):
        ...
        if self.rfp_id and self.product_id:
            ...
            existing_product_ids = self.rfp_id.product_line_ids.filtered(
                lambda l: l != self).mapped('product_id.id')
